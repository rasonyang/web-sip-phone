...
- **Reverse proxy must target the `wss` binding, not the plain `ws` one.** SIP.js stamps
  ...
  arrived on. A proxy that terminates TLS and forwards plain `ws` to the `ws-binding` port
  produces exactly that mismatch — registration times out with FreeSWITCH showing the REGISTER
  in `siptrace` but never answering. Re-encrypt to the `wss-binding` port instead, e.g. Caddy:

  ```
  ws.example.com {
      reverse_proxy https://<fs-ip>:7443 {
          transport http {
              tls
              tls_insecure_skip_verify   # FS's autogenerated wss.pem is self-signed
          }
      }
  }
  ```

  Note sofia binds `ws-binding`/`wss-binding` on the profile's `sip-ip` (usually the LAN
  address), not on loopback — point the proxy at that IP.
